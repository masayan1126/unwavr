...
export function BacklogHistory({ tasks }: BacklogHistoryProps) {
    ...
    return (
        <div className="space-y-4">
            ...
            {history.length === 0 ? (
                <div className="text-sm p-4 text-center border rounded-md border-dashed">
                    完了した積み上げタスクはまだありません。
                </div>
            ) : (
                    ...
                    {history.map(task => (
                        <div key={task.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-3 border rounded-md bg-card/50 hover:bg-card/80 transition-colors">
                            <div className="flex-1 min-w-0">
                                ...
                                {task.description && <div className="text-xs truncate">{task.description}</div>}
                            </div>
                            <div className="text-xs whitespace-nowrap mt-2 sm:mt-0 sm:ml-4">
