        ...
        <div className="fixed inset-0 z-[1000] flex items-center justify-center">
          ...
          <div className="relative w-full max-w-md bg-background text-foreground border border-black/10 dark:border-white/10 rounded-lg shadow-lg p-6 mx-4">
            ...
            <div className="flex justify-end gap-3">
              ...
              <button className={`px-4 py-2 rounded text-sm ${opts.tone === 'danger' ? 'bg-red-600 text-white' : 'bg-foreground text-background'}`} onClick={() => onClose(true)}>{opts.confirmText ?? 'OK'}</button>
            </div>
          </div>
        </div>
      )}
